...
namespace SpatialIndexStructures.RTree
{
    public class RTree : ISpatialIndex
    {
        ...
        private static IEnumerable<ISpatialData> GetSpatialData(List<ControlNode> sourceData)
        {
            ...
            ICollection<ISpatialData> entries = new Collection<ISpatialData>();
        ...
        public IEnumerable<ControlNode> GetAllControlNodes()
        {
            ICollection<ControlNode> nodes = new Collection<ControlNode>();
            if (_root == null) return nodes;
            nodes.Add(_root);
            GetChildrenToCollection(_root, nodes);
            return nodes;
        }

        private static void GetChildrenToCollection(ControlNode parent, ICollection<ControlNode> nodes)
        {
            if (parent.IsLeaf()) return;
            foreach (var node in parent.Children)
            {
                nodes.Add(node);
                GetChildrenToCollection(node, nodes);
            }
        }

        public ControlNode GetRoot()
        {
            return _root;
        }
    }
